...
class ExampleROS2(Node):
    ...
    def timer_callback(self):
        response = requests.post(f"{self.url}/post", json={"ros2_example": "POST"})
        if response.status_code == 200:
            self.get_logger().info('Post Respone %r' % (response.json(),))
        else:
            self.get_logger().warn('Post Respone %r' % (response.json(),))

        
def main(args=None):
    rclpy.init(args=args)
    node = ExampleROS2("Client_POST")
    try:
        rclpy.spin(node)
